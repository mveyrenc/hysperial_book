...
StimulusReflex.configure do |config|
  ...
  # Available tokens: session_id, session_id_full, reflex_info, operation, id, id_full, mode, selector,
  ...
  # eg. if your connection is `identified_by :current_user` and your User model has an email attribute, you can
  #     access r.email (it will display `-` if the user isn't logged in)
  # Learn more at: https://docs.stimulusreflex.com/appendices/troubleshooting#stimulusreflex-logging
  #
  # config.logging = proc { "[#{session_id}] #{operation_counter.magenta} #{reflex_info.green} -> #{selector.cyan}
  #    via #{mode} Morph (#{operation.yellow})" }
